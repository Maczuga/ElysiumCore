    ...
    switch (m_deathState)
    {
        ...
        case ALIVE:
        {
            ...
            if (AI())
            {
                ...
                {
                    sLog.nostalrius("[Unit/AI] Crash mob %u map %u", GetEntry(), GetMapId());
                    sLog.nostalrius("%s", e.what());
                }
                m_AI_locked = false;
            }
...
void Creature::DisappearAndDie()
{
    if (!IsInWorld())
    {
        sLog.nostalrius("[CRASH][%s]DisappearAndDie: le mob n'est pas InWorld.", GetName());
        return;
    }
